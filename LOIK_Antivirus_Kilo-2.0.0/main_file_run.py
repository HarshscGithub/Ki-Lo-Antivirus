...
class User_info(tk.Frame):
	...
	#remaining_days = str(remaining_days) + "days"
        ...
        label9 = tk.Label(self, bg="lightblue" , text="Version 2.0.8", height = 6, width = 30 )
...
class Scan_Utility(tk.Frame):
    ...
    def __init__(self, parent, controller):
        ...
        self.percentage = 0
        Label(self,text="Please wait while we are doing a quick scan.",font=("Italic", 20)).pack()
        self.load = Label(self,text=f"Scaning...{self.percentage}%,",font=("Italic",20))
        self.load.pack()
        self.load_bar()

        button1 = Button(self, bg="lightpink" , text="Switch To Pro", command=switch_to_pro, height = 5, width = 20,font=("courier"))
        button1.pack()

    def load_bar(self):
        self.percentage +=1 #Edit 5
